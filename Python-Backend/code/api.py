...
# Look for templates in the 'templates' folder relative to this script
TEMPLATES_DIR = os.path.join(BASE_DIR, "templates")

# Store generated docs in 'generated_docs' relative to this script
OUTPUT_DIR = os.path.join(BASE_DIR, "generated_docs")
...
async def generate_offer_letter(data: OfferLetterData, background_tasks: BackgroundTasks):
    ...
    return {
        "success": True,
        "docx_url": f"/download/{result['docx_filename']}",
        "pdf_url": f"/download/{result['pdf_filename']}" if result.get('pdf_filename') else None
    }
...
async def generate_termination_letter(data: TerminationLetterData, background_tasks: BackgroundTasks):
    ...
    result = generate_document("Termination Letter.docx", template_data, "termination_letter")
    cleanup_old_files(background_tasks)
    return {"success": True, "docx_url": f"/download/{result['docx_filename']}", "pdf_url": f"/download/{result['pdf_filename']}"}
...
async def generate_certificate_endpoint(data: CertificateData, background_tasks: BackgroundTasks):
    ...
    generate_certificate(template_path, pdf_path, data.NAME, data.POSITION, data.DURATION)
    
    # Save a DOCX version too
    doc = Document(template_path)
    replace_placeholders(doc, {"{{NAME}}": data.NAME, "{{POSITION}}": data.POSITION, "{{DURATION}}": data.DURATION})
    doc.save(docx_path)

    cleanup_old_files(background_tasks)
    return {"success": True, "docx_url": f"/download/{docx_filename}", "pdf_url": f"/download/{pdf_filename}"}

@app.post("/generate/experience-letter/aiml")
async def generate_aiml_experience_letter(data: AIMLExperienceLetterData, background_tasks: BackgroundTasks):
    result = generate_document("Experince_AI_Template.docx", data.dict(), "aiml_experience_letter")
    cleanup_old_files(background_tasks)
    return {"success": True, "docx_url": f"/download/{result['docx_filename']}", "pdf_url": f"/download/{result['pdf_filename']}"}

@app.post("/generate/experience-letter/webdev")
async def generate_webdev_experience_letter(data: WebDevExperienceLetterData, background_tasks: BackgroundTasks):
    result = generate_document("Experience_Web_Template.docx", data.dict(), "webdev_experience_letter")
    cleanup_old_files(background_tasks)
    return {"success": True, "docx_url": f"/download/{result['docx_filename']}", "pdf_url": f"/download/{result['pdf_filename']}"}

@app.post("/generate/experience-letter/graphic-design")
async def generate_graphic_design_experience_letter(data: GraphicDesignExperienceLetterData, background_tasks: BackgroundTasks):
    result = generate_document("Experience_Graphic_Template.docx", data.dict(), "graphic_design_experience_letter")
    cleanup_old_files(background_tasks)
    return {"success": True, "docx_url": f"/download/{result['docx_filename']}", "pdf_url": f"/download/{result['pdf_filename']}"}
...
@app.get("/")
async def root():
    return {"message": "Document Generator API is running. Access /docs for documentation."}
